...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--model", type=str, help="Model name to create")
        parser.add_argument("--fields", type=str, nargs='+', help="Fields for the model")
    ...
    def handle(self, *args, **kwargs):
        ...
        app_class_name = f"{app_name.capitalize()}"
        ...
        if not os.path.exists(template_dir):
            ...
            for file_name in ["models.py", "serializers.py", "views.py", "__init__.py"]:
                with open(os.path.join(template_dir, file_name), 'w') as f:
        ...
        if kwargs.get("model") and kwargs.get("fields"):
            model_name = kwargs["model"].strip().title().replace(" ", "")
            field_list = kwargs["fields"]
            
            # Create field strings for the model
            fields = []
            for field in field_list:
                fields.append(f"    {field} = models.CharField(max_length=255, default='')")

            fields_str = "\n".join(fields) or "    pass"
            first_field = field_list[0] if field_list else 'id'
